...
@pytest.mark.asyncio
async def test_two_stage_skips_enrich_when_disabled() -> None:
    search_html = """
    <div data-widget="tileGridDesktop">
      <div class="tile-root">
        <a href="/product/printer-hp-1/">
    ...
    parser = TwoStageOzonParser()
    navigate_mock = AsyncMock(return_value=(search_html, None))
    ...
    with (
        ...
        patch("app.scrapers.two_stage_ozon.settings.ozon_enrich_enabled", False),
        patch("app.scrapers.two_stage_ozon.settings.ozon_browser_cache_enabled", False),
        patch("app.scrapers.two_stage_ozon.settings.ozon_broad_search_max", 36),
        patch("app.scrapers.two_stage_ozon.settings.ozon_ml_top_k", 1),
    ):
    ...
    assert status is None
    assert len(products) == 1
    assert navigate_mock.await_count == 1


@pytest.mark.asyncio
async def test_two_stage_enrich_stops_on_captcha() -> None:
    search_html = """
    <div data-widget="tileGridDesktop">
      <div class="tile-root">
        <a href="/product/printer-hp-1/"><span class="tsBody500Medium">Принтер HP</span></a>
        <span>12 481 ₽</span>
      </div>
      <div class="tile-root">
        <a href="/product/printer-hp-2/"><span class="tsBody500Medium">Принтер Canon</span></a>
        <span>9 000 ₽</span>
      </div>
    </div>
    """
    ...
    with (
        ...
        patch("app.scrapers.two_stage_ozon.settings.ozon_browser_cache_enabled", False),
        patch("app.scrapers.two_stage_ozon.settings.ozon_broad_search_max", 36),
        patch("app.scrapers.two_stage_ozon.settings.ozon_ml_top_k", 2),
    ):
        products, error, status = await parser.search("принтер hp", skip_cache=True)
